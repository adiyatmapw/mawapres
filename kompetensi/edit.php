...
$db = new Kompetensi();
$Kompetensi = $db->getKompetensi($_GET['id']);
?>
    ...
    <div class="container mt-4">
        <div class="row">
            ...
            <div class="m-3 p-4 col-lg-12">
                <div class="card">
                <div class="card-body">
                        <h5 class="card-title">Respon Kompetensi Integritas</h5>
                        <form action="proses.php?action=edit" method="POST">
                            <div class="mb-3">
                                <label for="exampleInputEmail1" class="form-label">Status</label>
                                <input type="hidden" name="id" class="form-control" value="<?= $Kompetensi['id'] ?>">
                                <select class="form-select" aria-label="Default select example" name="status">
                                    <option selected><?= $Kompetensi['status'] ?></option>
                                    <?php
                                    if($Kompetensi['status'] == "Pending"){
                                        echo'
                                        <option name="status" value="ACC">ACC</option>
                                        <option name="status" value="Ditolak">Ditolak</option>';
                                    } else if($Kompetensi['status'] == "ACC"){
                                        echo'
