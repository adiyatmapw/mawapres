...
$db = new HasilKarya();
$HasilKarya = $db->getHasilKarya($_GET['id']);
?>
                ...
                <div class="card-body">
                        ...
                        <form action="proses.php?action=edit" method="POST">
                            <div class="mb-3">
                                ...
                                <input type="hidden" name="id" class="form-control" value="<?= $HasilKarya['id'] ?>">
                                <select class="form-select" aria-label="Default select example" name="status">
                                    <option selected><?= $HasilKarya['status'] ?></option>
                                    <?php
                                    if($HasilKarya['status'] == "Pending"){
                                        echo'
                                        <option name="status" value="ACC">ACC</option>
                                        <option name="status" value="Ditolak">Ditolak</option>';
                                    } else if($HasilKarya['status'] == "ACC"){
                                        echo'
