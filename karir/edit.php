<?php
include '../model/Karir.php';
include '../config/auth.php';
include '../config/mahasiswa.php';

$db = new Karir();
$Karir = $db->getKarir($_GET['id']);
?>
    ...
    <div class="container mt-4">
        <div class="row">
            ...
            <div class="m-3 p-4 col-lg-12">
                <div class="card">
                <div class="card-body">
                        <h5 class="card-title">Respon Karir</h5>
                        <form action="proses.php?action=edit" method="POST">
                            <div class="mb-3">
                                <label for="exampleInputEmail1" class="form-label">Status</label>
                                <input type="hidden" name="id" class="form-control" value="<?= $Karir['id'] ?>">
                                <select class="form-select" aria-label="Default select example" name="status">
                                    <option selected><?= $Karir['status'] ?></option>
                                    <?php
                                    if($Karir['status'] == "Pending"){
                                        echo'
                                        <option name="status" value="ACC">ACC</option>
                                        <option name="status" value="Silahkan Registrasi">Silahkan Registrasi</option>';
                                    } else if($Karir['status'] == "ACC"){
                                        echo'
