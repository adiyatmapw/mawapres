...
$db = new SuratGagasan();
$SuratGagasan = $db->getSuratGagasan($_GET['id']);
?>
    ...
    <div class="container mt-4">
        <div class="row">
            ...
            <div class="m-3 p-4 col-lg-12">
                <div class="card">
                <div class="card-body">
                        <h5 class="card-title">Respon SuratGagasan</h5>
                        <form action="proses.php?action=edit" method="POST">
                            <div class="mb-3">
                                <label for="exampleInputEmail1" class="form-label">Status</label>
                                <input type="hidden" name="id" class="form-control" value="<?= $SuratGagasan['id'] ?>">
                                <select class="form-select" aria-label="Default select example" name="status">
                                    <option selected><?= $SuratGagasan['status'] ?></option>
                                    <?php
                                    if($SuratGagasan['status'] == "Pending"){
                                        echo'
                                        <option name="status" value="ACC">ACC</option>
                                        <option name="status" value="Ditolak">Ditolak</option>';
                                    } else if($SuratGagasan['status'] == "ACC"){
                                        echo'
